...
module.exports = class DevServer {
  ...
  listenCSR (webpackConfig, cfg, resolve) {
    ...
    console.log(cfg.devServer, webpackConfig)

    const aliases = {}

    for (const [key, value] of Object.entries(webpackConfig.resolve.alias)) {
      aliases[`/${key}/`] = value
    }

    const server = createServer({
      hostname: cfg.devServer.host,
      resolvers: [
        {
          alias: aliases
        }
      ]
    }).listen(cfg.devServer.port, () => {
      resolve()
